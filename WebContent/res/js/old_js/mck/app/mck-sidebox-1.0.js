...
function MobiComKit() {
	...
	function MckUtils() {
		...
		_this.initializeApp = function initializeApp(options) {
			...
			var data = "applicationId=" + options.appId + "&userId="
					+ options.userId;
			$.getJSON(MCK_BASE_URL + INITIALIZE_APP_URL + "?" + data, function(
					result, status) {
						...
						if (!options.beforeSend) {
							options.beforeSend = function(jqXHR) {
								...
								jqXHR.setRequestHeader("Authorization",
										"Basic " + AUTH_CODE);
								jqXHR.setRequestHeader("Application-Key",
										APPLICATION_ID);
							};
						}
					});
			...
			$(document)
					.on(
							...
							function() {
								...
								$("#mck-file-box").removeClass('vis').addClass(
										'n-vis');
								$mck_text_box.removeClass('mck-text-wf');
								$mck_text_box.attr("required", "");
								$("#mck-textbox-container").removeClass(
										'mck-textbox-container-wf');
	...
	function MckMessageService() {
		...
		_this.init = function init(options) {
						...
						if (event.keyCode == 13
							...
							if (window.getSelection) {
								var selection = window.getSelection(), range = selection
										.getRangeAt(0), br = document
										.createElement("br"), textNode = document
										.createTextNode("\u00a0"); //Passing " " directly will not end up being shown correctly
									...
									if (confirm("Are you sure want to delete all the conversation!")) {
										$
												.ajax({
													type : "get",
													url : MCK_BASE_URL
															+ "/rest/ws/mobicomkit/v1/message/delete/conversation.task",
													global : false,
													data : "userId=" + userId,
			...
			$(document).on(
					...
					function(e) {
						...
						mckMessageService.loadMessageList($(this)
								.data("mck-id"));
						mckMessageLayout.openConversation();
					});
		...
		_this.sendMessage = function sendMessage(messagePxy) {
			...
			if (!FILE_METAS) {
				...
				var $mck_msg_div = $("#mck-message-cell .mck-message-inner div[name='message']."
						+ randomId);
				mckMessageLayout.clearMessageField();
			}

			$("." + randomId + " .mck-message-status").removeClass(
					'mck-icon-ok-circle').addClass('mck-icon-time');
			$
					.ajax({
						type : "POST",
						...
						success : function(data, status, xhr) {
							if (data === 'error') {
								...
								if (!FILE_METAS) {
									...
									$("." + data + " .mck-message-status")
											.removeClass('mck-icon-time')
											.addClass('mck-icon-ok-circle')
											.attr('title', 'sent');
		...
		_this.loadMessageList = function loadMessageList(userId) {
			...
			if (typeof userId !== "undefined") {
				...
				$mck_msg_to.parent('.form-group').removeClass('vis').addClass(
						'n-vis');
				...
				$mck_conversation_title
						.html('<a href="#" role="link" class="mck-conversation-tab-link"><img src="'
								+ MCK_BASE_URL
								+ '/resources/sidebox/images/ic_action_backward.png" alt="Back"></a>&nbsp '
								+ displayName);
			} else {
				individual = false;
				pageSize = 50;
				...
				$mck_msg_to.parent('.form-group').removeClass('n-vis')
						.addClass('vis');
				if (typeof (Storage) !== "undefined") {
					var mckMessageArray = JSON.parse(sessionStorage
			...
			$
					.ajax({
						url : MCK_BASE_URL + MESSAGE_LIST_URL
								+ "?startIndex=0&pageSize=" + pageSize
								+ userIdParam,
						type : 'get',
						global : false,
						success : function(data, status) {
							...
							if (data + '' === "null"
								...
								if (individual) {
									...
									$mck_msg_inner
											.removeClass('mck-msg-w-panel');
									$mck_top_btn_panel.removeClass('vis')
											.addClass('n-vis');
									$mck_msg_inner
								...
								if (individual) {
									...
									$mck_top_btn_panel.removeClass('n-vis')
											.addClass('vis');
									...
									$mck_msg_inner.data('mck-id', "");
									mckMessageLayout
											.addContactsFromMessageList(data);
									sessionStorage.setItem('mckMessageArray',
		...
		_this.loadMoreMessages = function loadMoreMessages() {
			...
			if (typeof userId !== "undefined") {
				...
				var data = "userId=" + userId
						+ "&startIndex=0&pageSize=30&endTime="
						+ $mck_show_more.data('datetime');
							...
							success : function(data, status) {
								if (data + '' === "null"
									...
									$mck_loading.removeClass('vis').addClass(
											'n-vis');
									$mck_show_more_icon.removeClass('n-vis')
											.addClass('vis');
									$mck_show_more_icon.fadeOut(3000, function(
											e) {
										$mck_show_more_icon.removeClass('vis')
												.addClass('n-vis');
									});
								}
	...
	function MckMessageLayout() {
		...
		_this.addTooltip = function addTooltip(msgKeyString) {
			...
			$("." + msgKeyString + " .mck-icon-ok-circle")
					.attr('title', 'sent');
			$("." + msgKeyString + " .btn-forward").attr('title',
					'forward message');
			$("." + msgKeyString + " .mck-icon-delivered").attr('title',
					'delivered');
			$("." + msgKeyString + " .msgtype-outbox-cr").attr('title',
					'sent via Carrier');
			$("." + msgKeyString + " .msgtype-outbox-mck")
					.attr('title', 'sent');
			$("." + msgKeyString + " .msgtype-inbox-cr").attr('title',
					'received via Carrier');
			$("." + msgKeyString + " .msgtype-inbox-mck").attr('title',
					'recieved');
		};
		...
		_this.addMessage = function addMessage(msg, append) {
			...
			var replyMessageParameters = "'" + msg.deviceKeyString + "'," + "'"
					+ msg.to + "'" + ",'" + msg.contactIds + "'" + ",'"
					+ replyId + "'";
			...
			if (contactIdsArray.length > 0 && contactIdsArray[0]) {
				for (var i = 0; i < contactIdsArray.length; i++) {
					...
					if (typeof contact !== 'undefined') {
						...
						var rel = contact.rel;
						rel = typeof rel === 'undefined' || rel.length == 0 ? ""
								: ' [' + rel + ']';
						var contactNumber = "";
						if (individual == false) {
							contactNumber = tos[i];
			...
			append ? $.tmpl("messageTemplate", msgList).appendTo(
					"#mck-message-cell .mck-message-inner") : $.tmpl(
					"messageTemplate", msgList).prependTo(
			...
			if (msg.type == 6 || msg.type == 7) {
				$textMessage.html(mckMessageLayout.getIcon(msg.type)
						+ $textMessage.html());
				(msg.type == 6) ? $textMessage.addClass("call_incoming")
						: $textMessage.addClass('call_outgoing');
			}
			$textMessage.linkify({
				target : '_blank'
			});
			if (msg.fileMetaKeyStrings) {
				...
				$("." + replyId + " .mck-file-text").removeClass('n-vis')
						.addClass('vis');
				if ($textMessage.html() == "") {
		...
		_this.getDisplayNameFromMessage = function getDisplayNameFromMessage(
				message) {
			var contact = this
					.getContact('' + message.contactIds.split(",")[0]);
			var name = "";
			if (typeof contact === "undefined") {
				name = message.to;
			} else {
				name = typeof contact.displayName === "undefined" ? contact.value
						: contact.displayName;
			}
			return name;
		};
				...
				if (typeof msg.fileMetas.length === "undefined") {
					if (msg.fileMetas.contentType.indexOf("image") != -1) {
						if (msg.fileMetas.contentType.indexOf("svg") != -1) {
							return '<a href="#" role="link" class="file-preview-link fancybox-media fancybox"><img src="'
									+ MCK_BASE_URL
									+ FILE_PREVIEW_URL
									+ msg.fileMetaKeyStrings
									+ '" area-hidden="true" data-imgurl="'
									+ MCK_BASE_URL
									+ FILE_PREVIEW_URL
									+ msg.fileMetaKeyStrings + '"></img></a>';
						} else {
							return '<a href="#" role="link" class="file-preview-link fancybox-media fancybox"><img src="'
									+ msg.fileMetas.thumbnailUrl
									+ '" area-hidden="true" data-imgurl="'
									+ MCK_BASE_URL
									+ FILE_PREVIEW_URL
									+ msg.fileMetaKeyStrings + '"></img></a>';
						...
						return '<a href="'
								...
								+ '"'
								+ msg.fileMetaKeyStrings
								+ '" role="link" class="file-preview-link" target="_blank"><span class="file-detail"><div class="file-name"><i class="icon-paperclip"></i>&nbsp;'
								+ msg.fileMetas.name
								+ '</div>&nbsp;<div class="file-size">'
								+ mckFileService
										.getFilePreviewSize(msg.fileMetas.size)
								+ '</div></span></a>';
					}
				} else {
					if (msg.fileMetas[0].contentType.indexOf("image") != -1) {
						...
						return '<a href="'
								...
								+ msg.fileMetaKeyStrings
								+ '" role="link" class="file-preview-link" target="_blank"><span class="file-detail"><div class="file-name"><i class="icon-paperclip"></i>&nbsp;'
								+ msg.fileMetas[0].name
								+ '</div>&nbsp;<div class="file-size">'
								+ mckFileService
										.getFilePreviewSize(msg.fileMetas[0].size)
								+ '</div></span></a>';
					}
				}
		...
		_this.getContactImageLink = function getContactImageLink(contact) {
			if (contact.photoLink === "") {
				...
				return '<img src="' + MCK_BASE_URL
						+ '/contact.image?photoLink=' + contact.photoLink
						+ '"/>';
			}

		};

		_this.getContactImageByAlphabet = function getContactImageByAlphabet(
				name) {
			if (typeof name === 'undefined' || name === "") {
				return '<div class="mck-alpha-contact-image mck-alpha-user"><span class="mck-contact-icon"><img src="'
						+ USER_ICON_URL + '" alt=""></span></div>';
			}
			...
			if (first_alpha.match(letters)) {
				...
				return '<div class="mck-alpha-contact-image alpha_user"><span class="mck-contact-icon"><img src="'
						+ USER_ICON_URL + '" alt=""></span></div>';
			}
		};

		_this.addContactsFromMessageList = function addContactsFromMessageList(
				data) {
			if (data + '' === "null") {
				return;
			} else {
				$("#mck-message-cell .mck-message-inner")
						.html(
								'<ul id="mck-contact-list" class=" mck-contact-list nav nav-tabs nav-stacked"></ul>');
				if (typeof data.message.length === "undefined") {
					...
					$.each(data.message, function(i, data) {
						if (!(typeof data.to === "undefined")) {
							mckMessageLayout
									.addContactsFromMessage(data, false);
						}
					});
				}
			}
		...
		_this.addContactsFromMessage = function addContactsFromMessage(message,
				update) {
			var data = message;
		...
		_this.updateRecentConversationList = function updateRecentConversationList(
				contact, msg, update) {
			if ($("#li-" + contact.htmlId).length > 0) {
				var $mck_msg_part = $("#li-" + contact.htmlId
		...
		_this.updateContact = function updateContact(contact, msg) {
			var emoji_template = emoji.replace_unified(msg.message.substring(0,
					15));
			emoji_template = emoji.replace_colons(emoji_template);
			$("#li-" + contact.htmlId + " .mck-cont-msg-date").html(
					typeof msg.createdAtTime === 'undefined' ? ""
							: mckDateUtils.getTimeOrDate(
									msg ? msg.createdAtTime : "", true));
			$("#li-" + contact.htmlId + " .mck-cont-msg-wrapper").html(
					msg ? emoji_template : "");
			var latestCreatedAtTime = $('#mck-contact-list li:nth-child(1)')
					.data('msg-time');
			var $contactElem = $("#li-" + contact.htmlId);
			$contactElem.data('msg-time', msg ? msg.createdAtTime : "");
			if ((typeof latestCreatedAtTime === "undefined" || (msg ? msg.createdAtTime
					: "") >= latestCreatedAtTime)
					&& $("#mck-contact-list li").index($contactElem) != 0) {
		...
		_this.addContact = function addContact(contact, msg) {
			...
			if (typeof msg !== 'undefined') {
				var emoji_template = emoji.replace_unified(msg.message
						.substring(0, 15));
				emoji_template = emoji.replace_colons(emoji_template);
			}
			var displayName = "";
			...
			var contactList = [ {
				...
				msgCreatedDateExpr : msg ? mckDateUtils.getTimeOrDate(
						msg.createdAtTime, true) : "",
			...
			if (typeof latestCreatedAtTime === "undefined"
					|| (msg ? msg.createdAtTime : "") > latestCreatedAtTime) {
				$.tmpl("contactTemplate", contactList).prependTo(
						"#mck-contact-list");
			} else {
				$.tmpl("contactTemplate", contactList).appendTo(
						"#mck-contact-list");
			}
		};

		_this.getStatusIcon = function getStatusIcon(msg) {
			return '<i class="' + this.getStatusIconName(msg) + ' move-right '
					+ msg.keyString + '_status status-icon"></i>';
		};
		_this.getStatusIconName = function getStatusIconName(msg) {
			...
			if (msg.type == 3
					|| (msg.type == 1 && msg.source == 0)
					|| ((msg.sent == "true" || msg.sent == true)
							&& msg.type != 0 && msg.type != 4)) {
				return 'mck-icon-ok-circle';
			}

			if (msg.type == 5
					|| (msg.type == 1 && (msg.source == 1 || msg.source == 2))) {
		...
		_this.removeConversationThread = function removeConversationThread(
				userId) {
			sessionStorage.removeItem("mckMessageArray");
			var contact = mckMessageLayout.getContact(userId);
			var currentTabId = $("#mck-message-cell .mck-message-inner").data(
					'mck-id');
	...
	function MckFileService() {
			...
			$file_upload
					.fileupload({
						...
						submit : function(e, data) {
							...
							$textbox_container
									.addClass('mck-textbox-container-wf');
							$file_name.html('<a href="#">' + data.files[0].name
									+ '</a>');
							$file_size.html("("
									+ parseInt(data.files[0].size / 1024)
									+ " KB)");
							...
							if (data.files[0].name === $(
									"#mck-file-box .mck-file-lb a").html()) {
						...
						success : function(result, textStatus, jqXHR) {
							var fileExpr = mckFileService.getFilePreviewPath(
									result, $("#mck-file-box .mck-file-lb a")
											.html());
		...
		_this.getFilePreviewPath = function getFilePreviewPath(
				fileMetaKeyString, fileName) {
			var name = (fileName) ? fileName : "file_attached";
			if (fileMetaKeyString) {
				return '<a href="' + FILE_PREVIEW_URL + fileMetaKeyString
	...
	function MckNotificationService() {
		...
		_this.notifyUser = function notifyUser(message) {
			...
			if (!IS_MCK_NOTIFICATION) {
				...
				if ($.browser.chrome) {
					var c_version = parseInt(window.navigator.appVersion
							.match(/Chrome\/(\d+)\./)[1], 10);
					if (c_version >= 35) {
						if (!Notification) {
							return;
					...
					if (window.webkitNotifications.checkPermission() === 0) {
						var notification = window.webkitNotifications
								.createNotification("/favicon.ico", name,
										message);
		...
		_this.showNewMessageNotification = function showNewMessageNotification(
				message) {

			if ($mck_sidebox.css('display') === 'none') {
				var contact = mckMessageLayout.getContact(''
						+ message.contactIds.split(",")[0]);
				if (typeof contact === 'undefined') {
					contact = mckMessageLayout.createContact(''
							+ message.contactIds.split(",")[0]);
				}
				...
				$mck_sidebox_launcher
						.addClass('mck-sidebox-launcher-with-preview');
			...
			setTimeout(function() {
				...
				$mck_sidebox_launcher
						.removeClass('mck-sidebox-launcher-with-preview');
			}, 10000);
		};
	...
	onMessage = function(response) {
		...
		if (messageType === "DELETE_SMS_CONTACT") {
			...
			$("." + resp.message.split(",")[0] + " .mck-message-status")
			...
			var userId = $("#mck-message-cell .mck-message-inner").data(
					'mck-id');
			...
			if (mckContactListLength > 0) {
				...
				if (messageType === "SMS_RECEIVED") {
					...
					if (typeof contact !== 'undefined') {
						...
						var userId = $("#mck-message-cell .mck-message-inner")
								.data('mck-id');
						if (typeof userId !== 'undefined'
					...
					if ((message.type !== 5 || message.source !== 1 || message.fileMetaKeyStrings)) {
						...
						if (contactIds.lastIndexOf(",") == contactIds.length - 1) {
							contactIds = contactIds.substring(0,
									contactIds.length - 1);
						}
						var contactIdsArray = contactIds.split(",");
						for (var i = 0; i < contactIdsArray.length; i++) {
							...
							if (typeof contact !== 'undefined') {
								...
								if (typeof userId !== 'undefined'
									...
									mckMessageLayout.addMessage(message, true);
									if (message.type == 3
											|| (message.type == 5 && message.fileMetaKeyStrings)) {
										$(
												"."
														+ message.keyString
														+ " .mck-message-status")
												...
												.addClass('mck-icon-ok-circle');
										mckMessageLayout
												.addTooltip(message.keyString);
									}
								}
							}
						}
					}
				} else if (messageType === "SMS_SENT_UPDATE"
						&& message.type != 0 && message.type != 4) {
					$("." + message.keyString + " .mck-message-status")
			...
			if (typeof (Storage) !== "undefined") {
				var mckLocalMessageArray = JSON.parse(sessionStorage
						.getItem('mckMessageArray'));
				if (mckLocalMessageArray != null) {
					mckMessageArray = mckMessageArray
							.concat(mckLocalMessageArray);
				}
			}
			sessionStorage.setItem('mckMessageArray', JSON
	...
	function MckDateUtils() {
		...
		_this.getDate = function getDate(createdAtTime) {
			...
			currentDate = new Date(parseInt(utcCurrentTime
					+ parseInt(MCK_USER_TIMEZONEOFFSET, 10)));
			return currentDate.getDate() !== date.getDate() ? dateFormat(date,
					fullDateFormat, false) : dateFormat(date, onlyTimeFormat,
					false);
		};
		...
		_this.getTimeOrDate = function getTimeOrDate(createdAtTime, timeFormat) {
			...
			currentDate = new Date(parseInt(utcCurrentTime
					+ parseInt(MCK_USER_TIMEZONEOFFSET, 10)));
			if (timeFormat) {
				return currentDate.getDate() !== date.getDate() ? dateFormat(
						date, onlyDateFormat, false) : dateFormat(date,
